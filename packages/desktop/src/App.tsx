...
function App() {
  ...
  const [currentView, setCurrentView] = useState<'chat' | 'overview' | 'market' | 'portfolio' | 'skills' | 'osskills' | 'defikeys' | 'marketoracles' | 'rpcconfig' | 'search' | 'playbooks'>('chat');
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="sidebar-scroll-area">
          <nav className="sidebar-nav" style={{ paddingTop: '16px' }}>
            ...
            <div 
              ...
              title={isSidebarCollapsed ? "Market" : undefined}
            >
              <LineChart size={15} className="nav-icon" /> <span className="nav-label">Market</span>
            </div>
            ...
            >
              <Wallet size={15} className="nav-icon" /> <span className="nav-label">Wallet</span>
            </div>
            ...
            >
              <BookOpen size={15} className="nav-icon" /> <span className="nav-label">Playbooks</span>
            </div>
        ...
        <nav className="sidebar-nav" style={{ paddingBottom: '16px', paddingTop: '8px' }}>
          <div 
      ...
      <main className="main-content">
          ...
          rightChildren={
            <>
              {!config ? (
                ...
                <div style={{ transform: 'scale(0.85)', transformOrigin: 'right center', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <button 
                    className="network-selector-pill theme-toggle-btn" 
                    style={{ 
                      padding: 0, 
                      borderRadius: '50%', 
                      background: 'transparent', 
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingLeft: '8px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Nyxora</span>
      ...
      {isSettingsOpen && (
        <div className="settings-modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', justifyContent: 'center', alignItems: 'center', backdropFilter: 'blur(2px)' }}>
          <div className="settings-modal-content" style={{ width: '95%', maxWidth: '1200px', height: '85vh', backgroundColor: 'var(--bg-color)', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 40px rgba(0,0,0,0.3)', border: '1px solid var(--glass-border)', position: 'relative' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-secondary)' }}>
              <h2 style={{ margin: 0, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}><SettingsIcon size={20} /> Settings</h2>
              <button 
