...
interface Config {
  ...
  llm: { provider: string; model: string; temperature: number; base_url?: string; reasoning_effort?: 'low' | 'medium' | 'high'; image_provider?: string; image_model?: string; };
          ...
          {activeCategory === 'llm' && (
            <div className="panel" style={{ background: 'transparent', border: 'none', padding: 0 }}>
              ...
              <div className="form-row" style={{ marginTop: '16px' }}>
                ...
                <div className="form-group flex-1">
                  <label className="nord-label">Image Model Name</label>
                  <input 
                    className="nord-pill-input"
                    type="text" 
                    placeholder={formData.llm.image_provider === 'gemini' ? "e.g. gemini-3.1-flash-image" : "e.g. dall-e-3"}
                    value={formData.llm.image_model || ''} 
                    onChange={e => handleChange('llm', 'image_model', e.target.value)} 
                  />
                </div>
              </div>
