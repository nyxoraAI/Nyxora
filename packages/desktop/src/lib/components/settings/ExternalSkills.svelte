...
{#if isLoading}
  <div class="flex items-center justify-center py-16 text-gray-500 dark:text-gray-400 text-xs">
    ...
    Loading external third-party skills...
  </div>
{:else}
  <div class="space-y-7 w-full">
    <div>
      <h2 class="text-xl font-bold tracking-tight text-gray-900 dark:text-gray-100 mb-1">External Skills</h2>
      <p class="text-xs text-gray-500 dark:text-gray-400 mb-6">User-installed third-party agent skills and custom integrations.</p>
      ...
      <div class="text-[0.75rem] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 px-1">Custom Capabilities</div>
        ...
        {#each filteredSkills as skill (skill.function.name)}
          ...
          <div class="flex items-center justify-between p-4 hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors">
            <div class="flex items-center gap-3.5 flex-1 pr-4">
              <div class="w-8 h-8 rounded-xl {isActive ? 'bg-violet-500/10 text-violet-500 dark:bg-violet-500/20 dark:text-violet-400' : 'bg-gray-200/60 text-gray-400 dark:bg-white/5 dark:text-gray-500'} flex items-center justify-center flex-shrink-0 transition-colors">
