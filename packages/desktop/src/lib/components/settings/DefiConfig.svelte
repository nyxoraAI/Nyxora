<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      console.error("Failed to fetch defi keys", err);
    }
  });
...
<div class="space-y-7 w-full">
  <div>
    <h2 class="text-xl font-bold tracking-tight text-gray-900 dark:text-gray-100 mb-1">DeFi Configuration</h2>
    <p class="text-xs text-gray-500 dark:text-gray-400 mb-6">Manage API keys for DeFi integrations and aggregators.</p>
    ...
    <div class="bg-amber-50/80 dark:bg-amber-900/15 border border-amber-200/80 dark:border-amber-900/40 p-4 rounded-2xl mb-5 flex gap-3.5 items-start">
      ...
      <p class="m-0 text-xs text-amber-800 dark:text-amber-200/90 leading-relaxed">
        ...
        <code class="bg-amber-100/80 dark:bg-black/30 px-1.5 py-0.5 rounded text-amber-900 dark:text-amber-400 font-mono">~/.nyxora/config/defi_keys.yaml</code>. 
    ...
    <div class="mb-2">
      <div class="text-[0.75rem] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 px-1">Dynamic Provider Integrations</div>
      <p class="text-xs text-gray-500 dark:text-gray-400 mb-4 px-1">These fields are dynamically loaded from installed DeFi Aggregator Providers. Provide your API keys to bypass rate limits.</p>
