    ...
    <div className="overview-container">
      ...
      <div style={{ background: 'rgba(235, 203, 139, 0.1)', border: '1px solid rgba(235, 203, 139, 0.3)', padding: '16px', borderRadius: '8px', marginBottom: '32px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
        ...
        <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
          ...
          They will simply appear as <code style={{ background: 'var(--bg-secondary)', padding: '2px 6px', borderRadius: '4px', color: 'var(--accent)' }}>IS_SET</code>.
        </div>
      </div>
      ...
      <div className="settings-section">
        <h3 className="section-title" style={{ color: 'var(--accent)', marginBottom: '8px', fontSize: '1.2rem' }}>Protocol Integrations</h3>
        <p className="section-description" style={{ marginBottom: '32px', color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Provide your own API keys to bypass public rate limits and unlock full Meta-Aggregator speed.</p>
        
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
          {DEFI_PROTOCOLS.map(protocol => {
            const isSet = keys[protocol.id] === 'IS_SET';
            return (
              <div key={protocol.id} style={{ background: 'var(--bg-secondary)', padding: '20px', borderRadius: '12px', border: '1px solid var(--glass-border)', display: 'flex', gap: '24px', alignItems: 'center' }}>
                <div style={{ width: '280px', flexShrink: 0, display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                  <img src={getRouterLogoUrl(protocol.id)} alt={protocol.id} style={{ width: 32, height: 32, borderRadius: '50%', marginTop: '2px' }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <strong style={{ color: 'var(--text-primary)', fontSize: '1rem', display: 'block' }}>{protocol.name}</strong>
                    ...
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', lineHeight: 1.4, display: 'block' }}>{protocol.desc}</span>
                  </div>
                </div>
                ...
                <div style={{ flex: 1, display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <div style={{ position: 'relative', flex: 1 }}>
                    <input
                      ...
                      style={{ paddingRight: '40px', borderColor: isSet ? '#a3be8c' : 'var(--glass-border)', width: '100%' }}
