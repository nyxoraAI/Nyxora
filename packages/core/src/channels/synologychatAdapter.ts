...
export class SynologychatAdapter implements ChannelAdapter {
    id: string = 'synologychat';
    name: string = 'Synologychat';
    private server: any;
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.synologychat?.port || 3009;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.synologychat`);
    }
}
export const adapter = new SynologychatAdapter();
