...
export class VoicecallAdapter implements ChannelAdapter {
    id: string = 'voicecall';
    name: string = 'Voicecall';
    private server: any;
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.voicecall?.port || 3013;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.voicecall`);
    }
}
export const adapter = new VoicecallAdapter();
