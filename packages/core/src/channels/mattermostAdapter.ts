...
export class MattermostAdapter implements ChannelAdapter {
    id: string = 'mattermost';
    name: string = 'Mattermost';
    private server: any;
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.mattermost?.port || 3007;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.mattermost`);
    }
}
export const adapter = new MattermostAdapter();
