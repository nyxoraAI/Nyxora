...
export class NextcloudtalkAdapter implements ChannelAdapter {
    id: string = 'nextcloudtalk';
    name: string = 'Nextcloudtalk';
    private server: any;
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.nextcloudtalk?.port || 3010;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.nextcloudtalk`);
    }
}
export const adapter = new NextcloudtalkAdapter();
