...
export class TwitchAdapter implements ChannelAdapter {
    ...
    async start(): Promise<void> {
        ...
        const token = config.integrations?.twitch?.token;
        if (!token) {
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.twitch`);
    }
}
export const adapter = new TwitchAdapter();
