...
export class IrcAdapter implements ChannelAdapter {
    ...
    async start(): Promise<void> {
        ...
        const token = config.integrations?.irc?.token;
        if (!token) {
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.irc`);
    }
}
export const adapter = new IrcAdapter();
