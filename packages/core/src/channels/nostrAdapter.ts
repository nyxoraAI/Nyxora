...
export class NostrAdapter implements ChannelAdapter {
    ...
    async start(): Promise<void> {
        ...
        const token = config.integrations?.nostr?.token;
        if (!token) {
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.nostr`);
    }
}
export const adapter = new NostrAdapter();
