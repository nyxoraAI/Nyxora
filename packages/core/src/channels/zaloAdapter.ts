...
export class ZaloAdapter implements ChannelAdapter {
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.zalo?.port || 3011;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.zalo`);
    }
}
export const adapter = new ZaloAdapter();
