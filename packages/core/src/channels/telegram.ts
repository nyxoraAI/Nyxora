...
function createStreamBubble(ctx: any, replyToMsgId?: number): StreamBubble {
  // ── State ──────────────────────────────────────────────────────────────────
  let buffer = '';
  let textMsgId: number | null = null;      // Bubble A: frozen initial LLM response
  let progressMsgId: number | null = null;  // Bubble B: progress + final answer
  let textBubbleFrozen = false;             // true once first tool call freezes Bubble A
  ...
  const sendNew = async (html: string): Promise<number | null> => {
    ...
    try {
      const sent = replyToMsgId
        ? await ctx.reply(html, { parse_mode: 'HTML', reply_parameters: { message_id: replyToMsgId } as any })
  ...
  /** Edit Bubble A (initial text). No-op if already frozen or doesn't exist. */
  const editTextBubble = async (html: string): Promise<void> => {
    if (textBubbleFrozen || !html || html.trim() === '') return;
    if (!textMsgId) {
  ...
  const onChunk = (chunk: string): void => {
    enqueue(async () => {
      ...
      if (chunk === '[CLEAR_STREAM]') {
        ...
        // IMPORTANT: do NOT create Bubble B here — it must always appear BELOW
        // Bubble A. Bubble A is created first by the streaming text.
        // Only EDIT Bubble B if it already exists (turns 2+).
        if (pendingFlushTimer) { clearTimeout(pendingFlushTimer); pendingFlushTimer = null; }
        if (!textBubbleFrozen && buffer.trim()) {
          // Flush any accumulated text to Bubble A before resetting
          await editTextBubble(formatToTelegramHTML(buffer));
        }
        buffer = '';
        lastProgressHtml = '';
        // Only edit Bubble B if it was already created (turn 2+)
        if (progressMsgId) {
      ...
      // scheduleFlush() internally routes to Bubble A (frozen=false) or Bubble B (frozen=true).
