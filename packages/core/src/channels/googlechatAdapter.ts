...
export class GooglechatAdapter implements ChannelAdapter {
    id: string = 'googlechat';
    name: string = 'Googlechat';
    private server: any;
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.googlechat?.port || 3008;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.googlechat`);
    }
}
export const adapter = new GooglechatAdapter();
