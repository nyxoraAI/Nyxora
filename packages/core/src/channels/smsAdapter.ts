...
export class SmsAdapter implements ChannelAdapter {
    ...
    async start(): Promise<void> {
        ...
        const port = config.integrations?.sms?.port || 3012;
    ...
    async setupCredentials(config: any): Promise<void> {
        console.log(`[Setup] Configure ${this.name} in config.yaml under integrations.sms`);
    }
}
export const adapter = new SmsAdapter();
