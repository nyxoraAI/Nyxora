...
version: 0.0.1
author: xdevplatform + openclaw + Nyxora Agent
license: MIT
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Nyxora tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Nyxora Docker layout, `/opt/data` is `NYXORA_HOME`, but Nyxora tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Nyxora-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Nyxora tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Nyxora subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Nyxora adaptation: reformatted for Nyxora skill conventions; safety guardrails preserved verbatim.
