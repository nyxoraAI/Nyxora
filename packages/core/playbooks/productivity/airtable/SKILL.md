...
- **`AIRTABLE_API_KEY` flows from `${NYXORA_HOME:-~/.nyxora}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
