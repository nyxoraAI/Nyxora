...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _nyxora_env="${NYXORA_HOME:-$HOME/.nyxora}/.env"; [ -f "$_nyxora_env" ] && grep -q "^GITHUB_TOKEN=" "$_nyxora_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_nyxora_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Nyxora Agent\",
    \"comments\": [
...
```bash
source "${NYXORA_HOME:-$HOME/.nyxora}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Nyxora Agent. Code looks clean — good test coverage, no security concerns."
