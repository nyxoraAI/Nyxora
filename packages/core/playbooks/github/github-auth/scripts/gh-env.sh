#!/usr/bin/env bash
# GitHub environment detection helper for Nyxora Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _nyxora_env="${NYXORA_HOME:-$HOME/.nyxora}/.env"; [ -f "$_nyxora_env" ] && grep -q "^GITHUB_TOKEN=" "$_nyxora_env" 2>/dev/null; then
