...
NYXORA_HOME_DIR="${NYXORA_HOME:-$HOME/.nyxora}"
NYXORA_CFG="${NYXORA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nyxora config has twozero_td MCP entry ──
if [[ ! -f "$NYXORA_CFG" ]]; then
    echo -e " ${FAIL} Nyxora config not found at ${NYXORA_CFG}"
    manual_steps+=("Create ${NYXORA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NYXORA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nyxora config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nyxora config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NYXORA_CFG} manually"); }
    manual_steps+=("Restart Nyxora session to pick up config change")
fi
