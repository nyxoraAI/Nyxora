...
## 5. Nyxora Configuration

### Config location

`$NYXORA_HOME/config.yaml` (defaults to `~/.nyxora/config.yaml` when `NYXORA_HOME` is unset)
...
Restart the Nyxora session for changes to take effect. The MCP connection is
