...
- **Dashboard (Frontend UI/UX):** Eradicated massive WebSocket connection overhead by extracting `io()` instantiation into a dedicated singleton `socket.ts`. Remedied API state update race conditions on the `Settings.tsx` page via unified `Promise.all` mounts. Fixed an undocumented API crash in `analyzeDocument.ts` when parsing `.xlsx` spreadsheets.
