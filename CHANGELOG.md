...
- **Agent Subsystems:** Implemented real-time cache invalidation for `disabled_skills.json`. Fixed `isSkillActive()` checks for web3 plugin isolation. Restructured token pricing logic in `tracker.ts` to accurately handle isolated input/output models. Removed dead null-checks and unreachable throws across `setup.ts` and `reasoning.ts`. Fixed `decryptDataSync` base64 delimiter parsing. Reintroduced Mnemonic display on first wallet generation and stopped zombie Telegram bots upon initialization errors. Fixed regex symbol extraction to retain hyphens and periods in `episodic.ts`.
- **Policy Engine Types:** Fixed undefined configuration edge cases in `setup.ts` and strict array casting in `analyzeDocument.ts`. Fixed a critical syntax bracket error in `policy/server.ts` JWT validation.
- **Core Engine (System & Memory):** Fixed Auto-Migration saving plaintext credentials to disk in `parser.ts` (explicitly re-encrypted). Patched a severe SQL Injection vulnerability in `logger.ts` via parameterized queries. Removed arbitrary RCE vector in `pluginManager.ts`. Mitigated Directory Traversal attacks in `logger.ts` and `episodicDB.ts`. Added explicit terminal warnings for fallback encryption key usage. Fortified `paths.ts` against path traversal. Resolved memory leak and premature aborts in `httpClient.ts`. Encapsulated tool_call JSON parsing in `logger.ts` within `try-catch`. Fixed an interval memory leak in `transactionManager.ts`. Secured `bridgeWatcher.ts` with comprehensive Promise rejection catching. Handled all withdrawal statuses in `bridgeWatcher.ts` to prevent stuck claims. Fixed BigInt precision loss for large amounts. Prevented critical keyword stripping in `episodic.ts`. Capped reflection history context to prevent privacy leaks. Persisted tracker statistics across restarts via `stats.json`. Implemented AES-256-GCM encryption for `defi_keys.yaml` to secure plaintext API keys. Corrected UI synchronization event emission upon memory wipes in `logger.ts`. Wrapped prompt injections (`user.md`, `policy.yaml`) within XML tags in `reasoning.ts` to prevent injection attacks. Added strict existence checks for pending transactions. Implemented atomic rename operations for `user.md` to prevent data corruption.
- **AI Reasoning & Formatting:** Mitigated Persistent XSS vulnerabilities by implementing strict HTML escaping inside `formatter.ts` and `telegram.ts`. Added defensive guard clauses (`?.`) in `reasoning.ts` to gracefully alert the user when LLM APIs trigger rate limits. Fixed an uppercase string formatting crash in `formatter.ts`. Patched reversed `fastReturnAll` logic for getBalance and checkPortfolio.
- **Gateway, Policy & API:** Patched Broken Access Control on `/api/profile` by enforcing strict session validation. Mitigated CSRF attacks on the local API gateway via `SameSite=strict` cookies and strict CORS matching. Hardened JWTs against Replay Attacks and Session Fixation. Removed the `DELETE /api/auth/google` route from the unauthorized bypass list. Fixed transaction reject state inconsistency in `telegram.ts`. Implemented strict JWT service claim validation in `policy/server.ts`. Unified HMAC secret generation in `vaultClient.ts`. Injected `memoryEvents.emit` into the background execution pipeline for real-time updates. Rejected duplicate Telegram callback payloads for non-pending transactions. Emitted socket updates for all transaction status transitions. Mitigated chmod race condition during Unix socket creation using `process.umask(0o177)`.
- **Dashboard (Frontend UI/UX):** Eradicated massive WebSocket connection overhead by extracting `io()` instantiation into a dedicated singleton `socket.ts`. Remedied API state update race conditions on the `Settings.tsx` page via unified `Promise.all` mounts. Fixed an undocumented API crash in `analyzeDocument.ts` when parsing `.xlsx` spreadsheets. Refactored `messages.length` state checking using functional updates in `App.tsx` to prevent stale closure bugs. Cleared `fetchConfig` timeout hooks upon component unmount to plug memory leaks. Switched `handleApproveAll` to execute sequentially to prevent nonce collisions.
...
- **Telegram Gateway:** Patched a critical Replay Attack vulnerability. The Telegram callback handler now strictly matches and invalidates transaction nonces upon execution. Additionally, injected the `checkRegistryStatus()` Kill-Switch validation directly into the Telegram callback pipeline to enforce Decentralized Governance parity with the core API.
- **MCP Server Architecture:** Resolved a severe security flaw where the MCP Server bypassed the Core Engine's Zero-Trust Queue. MCP traffic is now correctly routed to the Core Engine (Port 3000) via a new `POST /api/transactions` endpoint, forcing external AI requests to enter the manual approval queue rather than attempting unauthorized auto-execution.
...
- **File System:** Fixed an operator precedence bug in `paths.ts` that caused `.json` memory files to be incorrectly categorized.
- **Database Encapsulation:** Removed direct `logger['db']` private access across the codebase. Limit order database operations in `eventListener.ts` are now securely encapsulated within public methods (`getActiveLimitOrders` and `updateLimitOrderStatus`).
- **Token Normalization:** Standardized native token equality checks across 14 distinct files using a centralized, backward-compatible `isNativeToken(address)` helper, resolving deep-seated inconsistencies between `0x00...` and `0xee...` placeholder addresses.
- **Oracle Routing:** Fixed the `estimateUsdValue()` routing fallback. `BNB` and `MATIC` now correctly route to `WBNB` and `WMATIC` instead of inaccurately defaulting to `USDC` for price evaluations.
- **UI & UX:** Restored broken block explorer verification links in the `SwapWidget.tsx` by implementing a dynamic chain-based URL constructor. Removed hardcoded `id-ID` TTS language bindings to support global accessibility.
...
- **Singleton Pattern:** Fixed a duplicated instantiation of the `Logger` class in `reasoning.ts` that spawned concurrent SQLite connections and triggered `SQLITE_BUSY` database locks. The logger is now strictly re-exported from its core module.
- **Process Management:** Removed aggressive `pkill -f ts-node` routines and duplicate `SIGTERM` listeners from `launcher.ts`, deferring entirely to the existing graceful PID-based termination flow.
- **Signer Endpoint:** Implemented the missing `POST /sign-typed-data` endpoint in the Signer Vault to support `EIP-712` payloads gracefully.
