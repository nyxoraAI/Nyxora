...
*   **Background Sub-Agents:** Nyxora can spawn detached background agents (`createMarketWatchAgent`) to continuously monitor market conditions, prices, or liquidity pools. Tugas Market Watch kini bersifat persisten (*persistent*) dan riwayat *task* akan tersimpan otomatis di *file* `~/.nyxora/data/market_tasks.json`, memungkinkannya melanjutkan pekerjaan bahkan setelah daemon di-restart.
...
*   **DEX LP Manager:** Integrated Uniswap V3 (and PancakeSwap V3 for BSC) liquidity provision with strict human-in-the-loop safety barriers for `tickLower` and `tickUpper` price ranges. Saat ini sistem juga telah mendukung opsi otomatis **Full Range** apabila parameter `tickLower` dan `tickUpper` tidak diberikan, sangat memudahkan pengguna awam.
