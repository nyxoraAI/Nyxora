...
These skills are loaded natively from the core engine via `packages/core/src/system/plugins/`. They allow the AI to interact directly with your local machine, file system, and terminal.
...
*   **`generate_download_link`**
    *   **Description:** Generates a secure, temporary HTTP endpoint (`/api/download`) for a local file.
    *   **Capabilities:** Allows the AI to serve files (e.g., generated PDFs, Excel reports, or zip archives) directly to the user via the Web Dashboard or chat interfaces.

### 📓 Note Management (Personal Memory)
*   **`save_note`, `read_note`, `list_notes`, `delete_note`**
    *   **Description:** Manages a persistent collection of textual notes.
    *   **Capabilities:** Allows the AI to act as your personal "Keep" or notebook, storing structured or unstructured thoughts, API keys, addresses, or to-do lists for later semantic retrieval.
