...
The Policy Engine acts as a strict middleware firewall between the Brain and the Vault. It primarily communicates via Hyper-Optimized IPC (Unix Socket) to prevent local TCP network intervention, using `/tmp/nyxora-policy.sock` exclusively.
